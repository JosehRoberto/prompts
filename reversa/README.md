...
## Problema
Ao executar `npx reversa update` mensagem **"Reversa: Update Reversa not installed in this directory"** aparece mesmo após `npx reversa install`. O motivo costuma ser `state.json` invalido (vírgulas/ chaves ausentes) devido a interrupção ou bug ao salvar.
